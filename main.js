...
(function() {
  ...
  const satData = [{radius:0.7,speed:0.8,size:0.04,color:0x00D9F5,tilt:Math.PI/2},{radius:1.0,speed:0.5,size:0.05,color:0x7B2FBE,tilt:Math.PI/3},{radius:1.35,speed:0.35,size:0.035,color:0xF0F6FF,tilt:Math.PI/1.6},{radius:1.7,speed:0.25,size:0.04,color:0x7B2FBE,tilt:Math.PI/4}];
  const satellites = satData.map((d,i) => {
    ...
    return { mesh, ...d, angle: i*Math.PI/2 };
  });
